...
class CRACK500CROPTSegmentation(data.Dataset):
    def __init__(
            self,
            dataset=None,
            path=None,
        ...
        if self.use_points:
            for label_line in self.img_ids:
                ...
                name = osp.join(
                    self.path, "gtFine/%s/%s_gtFine_labelIds_%s" % (self.split, label_line[:-26], label_line[-13:])
                )
        ...
        for name in self.img_ids:
            ...
            label_file = osp.join(
                self.path, "gtFine/%s/%s_gtFine_labelIds_%s" % (self.split, name[:-26], name[-13:])
            )
            self.files.append({
                "img": img_file,
                "label": label_file,
