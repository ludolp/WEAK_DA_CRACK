...
dropout = nn.Dropout(0.1)  # 0.1 and 0.3 for oracle-weak labels and pseudo-weak labels
...
class TrainerWeakda(Trainer):
    ...
    def training(self):
        ...
        for i_iter in range(self.args.num_steps):
            ...
            # print('interp_source', labels.shape[1], labels.shape[2])
            # self.interp_source = nn.Upsample(
            #     size=(labels.shape[1], labels.shape[2]), mode='bilinear', align_corners=True)
            ...
            if self.args.source_only:
                ...
                if self.args.use_pseudo:
                    ...
                    # print("pseudo label for ", name, ": ", weak_labels_onehot)
                else:
                    weak_labels_onehot = get_weak_labels(
                        labels_target, pred_target2.shape[1], self.target_th)
                    # print("image label for ", name, ": ", weak_labels_onehot)
                weak_labels_onehot = weak_labels_onehot.to(self.device)

                # self.interp_target = nn.Upsample(
                #     size=(labels_target.shape[1], labels_target.shape[2]), mode='bilinear', align_corners=True)
                # print('interp_target', labels_target.shape[1], labels_target.shape[2])

                if self.args.use_pointloss:
                    point_label_list = point_label_list.long().to(self.device)
                    # print("point label for ", name, ": ", point_label_list)
                    loss_point = []
                ...
                loss = self.args.lambda_adv_target1 * loss_adv_target1 + \
                    self.args.lambda_adv_target2 * loss_adv_target2  # output space alignment
                loss += self.args.lambda_weak_target2 * loss_weak_target2 + loss_point.cuda() + \
                    self.args.lambda_weak_cwadv2 * loss_weak_cwadv2  # Lc, point loss, Ladv (segmentation network)
            ...
            if i_iter % self.args.print_loss_every == 0 and i_iter < self.args.num_steps - 1:  ###
                ...
                # writer.add_scalar("Loss_G/L_s", loss_seg_value2, i_iter)
                # writer.add_scalar("Loss_G/L_cw", loss_weak_cwadv_value2, i_iter)
                # writer.add_scalar("Loss_G/L_c", loss_weak_target2, i_iter)
                # writer.add_scalar("Loss_G/L_adv", loss_adv_target_value2, i_iter)
                # writer.add_scalar("Loss_G/L_p", loss_point_value, i_iter)
                # writer.add_scalar("Loss_D/L_d", loss_D_value2, i_iter)
